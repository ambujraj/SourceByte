...
def write_to_result(df, filename):
    if get_common_value("WriteToS3"):
        output_filename = filename
        write_to_s3(df, output_filename)
    else:
        output_filename = '../data/{}/result/{}'.format(process_id, filename)
...
def read_input():
    staged_file_exists = path.exists('../data/{}/staged/staged.xlsx'.format(process_id))
    if not staged_file_exists:
        return post_read_processing()

    else:
        log.info("Reading from staged...")
        df = pd.read_excel('../data/{}/staged/staged.xlsx'.format(process_id), sheet_name='Data')
